...
class CanvasSettingsRepository @Inject constructor(
    ...
    val configuration: Flow<CanvasConfiguration> = context.dataStore.data.map { preferences ->
        CanvasConfiguration(
            ...
            sourceOrder = parseCanvasSourceOrder(preferences[CanvasSourceOrderKey]),
    ...
    suspend fun setSourceOrder(order: List<CanvasSource>) {
        context.dataStore.edit { it[CanvasSourceOrderKey] = order.joinToString(",") { source -> source.name } }
    }
...
private fun parseCanvasSourceOrder(raw: String?): List<CanvasSource> {
    if (raw.isNullOrBlank()) return defaultCanvasSourceOrder
    val parsed = raw.split(",").mapNotNull { name ->
        runCatching { CanvasSource.valueOf(name.trim()) }.getOrNull()
    }.filter { it != CanvasSource.ALL }.distinct()
    val missing = defaultCanvasSourceOrder.filter { it !in parsed }
