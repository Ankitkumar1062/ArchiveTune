...
internal fun MusicMultiRowListItemRenderer.toEpisodeItem(): EpisodeItem? {
    ...
    val durationRun = subtitleRuns.lastOrNull { it.text.parseTime() != null }
    val durationText = durationRun?.text?.trim()
    return EpisodeItem(
        id = videoId,
        browseId =
            title
...
private fun Runs?.textOrNull(): String? =
    this?.runs?.joinToString(separator = "") { it.text }?.trim()?.takeIf(String::isNotBlank)

private fun BrowseEndpoint.isPodcastShowEndpoint(): Boolean =
    browseId.startsWith("MPSP") || browseId.startsWith("VL") ||
        browseEndpointContextSupportedConfigs?.browseEndpointContextMusicConfig?.pageType == "MUSIC_PAGE_TYPE_PODCAST_SHOW"

private fun BrowseEndpoint.isPodcastEpisodeEndpoint(): Boolean =
    browseId.startsWith("MPED") ||
        browseEndpointContextSupportedConfigs?.browseEndpointContextMusicConfig?.pageType == "MUSIC_PAGE_TYPE_PODCAST_EPISODE"
