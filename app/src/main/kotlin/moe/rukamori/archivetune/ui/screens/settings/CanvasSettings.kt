...
private fun CanvasSettingsBody(
    ...
    Column(
        ...
        CanvasSection(title = stringResource(R.string.canvas_source)) {
            ...
            if (model.configuration.source == CanvasSource.ALL) {
                Surface(
                    onClick = { onAction(CanvasSettingsAction.ShowSourceOrder) },
                    enabled = !model.busy,
                    ...
                    Row(
                        modifier = Modifier.fillMaxWidth().heightIn(min = 56.dp)
                            .padding(horizontal = SettingsDimensions.RowHorizontalPadding, vertical = 8.dp),
                        verticalAlignment = Alignment.CenterVertically,
                        horizontalArrangement = Arrangement.spacedBy(16.dp),
                    ) {
                        val context = LocalContext.current
                        val orderSummary = remember(model.configuration.sourceOrder) {
                            model.configuration.sourceOrder.joinToString(" → ") { context.getString(it.labelResource()) }
                        }
                        Column(modifier = Modifier.weight(1f)) {
                            Text(stringResource(R.string.canvas_priority_order), style = MaterialTheme.typography.bodyLarge)
                            Text(
                                text = orderSummary,
...
private fun CanvasSettingsDialogs(model: CanvasSettingsUiModel, onAction: (CanvasSettingsAction) -> Unit) {
    ...
    when (model.dialog) {
        ...
        CanvasSettingsDialog.SOURCE_ORDER -> CanvasSourceOrderDialog(
            initialOrder = model.configuration.sourceOrder,
            onDismiss = onDismiss,
            onConfirm = { newOrder ->
                onAction(CanvasSettingsAction.SetSourceOrder(newOrder))
                onAction(CanvasSettingsAction.DismissDialog)
            },
        )
        null -> Unit
    }
...
@Composable
private fun CanvasSourceOrderDialog(
    initialOrder: List<CanvasSource>,
    onDismiss: () -> Unit,
    onConfirm: (List<CanvasSource>) -> Unit,
) {
    ...
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(stringResource(R.string.canvas_priority_order)) },
        text = {
            LazyColumn(
                state = lazyListState,
                ...
                itemsIndexed(sources, key = { _, item -> item.name }) { index, source ->
                    ReorderableItem(reorderableState, key = source.name) {
                        ...
                        Row(
                            ...
                            Text(
                                text = stringResource(source.labelResource()),
                                style = MaterialTheme.typography.bodyMedium,
                                color = contentColor,
        ...
        confirmButton = {
            TextButton(onClick = { onConfirm(sources.toList()) }) {
